...
class Ini_Parser():
    ...
    def fromString(s, keysList=True, rootSection='_'):
        ...
        def putValue(self, r, section, key, value):
            _ss = Ini_Parser
            if key not in r[section]:
                r[section][key] = value

            else:
                if isinstance(r[section][key],list):
                    r[section][key].append(value)
                else:
                    r[section][key] = [r[section][key], value]
            ...
            # key-value pairs
                ...
                if '"'==linestartswith or "'"==linestartswith:
                    ...
                    if line.find('=', 0)>-1:
                        ...
                        putValue(_self, currentRoot, currentSection, key, value)
                ...
                if isinstance(keyvals, list):
                    ...
                    if len(keyvals):
                        ...
                        for k,v in keyvals.items():
                            ...
                            if not len(v):
                                if k == "__list__": continue
                                else: s += q+k+q+ '=' + EOL
                            
                            elif isinstance(v, dict) or isinstance(v, list):
                            
                                # sub-section
                                s += Ini_Parser.walk(keyvals, k, top + "[" + str(section) + "]", q, EOL, SUB=True)
